...
class ProxmoxApiComputeUtilSpec extends Specification {
    ...
    def "test requestVMConsole vnc success"() {
        ...
        def consoleDetails = [host: "::1", port: 5900, ticket: "vnc-ticket", user: "root@pam"]
        def mockApiResponse = new ServiceResponse(success: true, data: [data: consoleDetails])
        mockClient.callJsonApi(authConfig.apiUrl, "/api2/json/nodes/node1/qemu/200/vncproxy", null, null, _ as HttpApiClient.RequestOptions, 'POST') >> mockApiResponse
        ...
        then:
        response.success
        response.data.ticket == "vnc-ticket"
        response.data.type == "vnc"
        response.data.proxmoxHost == "localhost"
        response.data.proxmoxPort == 8006
        1 * mockClient.callJsonApi(authConfig.apiUrl, "/api2/json/nodes/node1/qemu/200/vncproxy", null, null, _ as HttpApiClient.RequestOptions, 'POST')
    }
